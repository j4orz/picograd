// //! picograd has a single tensor programming model (eager mode) following the
// //! python array API standard https://data-apis.org/array-api/latest/API_specification/index.html
// //! but whose execution semantics are either eagerly evaluated or lazy compiled
// //! depending on its backing execution engine. In the former case, tensor is a
// //! handle to a materialized view of device memory, whereas in the latter case,
// //! the tensor is a handle to a node in a graph of tinygrad UOps. To see the
// //! difference between how automatic differentiation is implemented for eager and
// //! lazy engines, refer to the engine module.

// use crate::engine::{self, Engine, Session, Storage};
// use crate::eagker::cpu::{forward_cpu, OpForwardError, ReduceDimInput};
// use crate::runtime::{Device, Dtype, DtypeVal, Layout};

// use std::ops::{Add, Div, Mul, Neg, Sub};
// use std::{collections::HashSet, hash, cmp::{Ordering, max}};
...
//     // binary
//     pub fn add(&self, other: Self, sess: Session) -> Self {
//         match sess.engine {
//         Engine::Eager(view) => todo!(),
//         Engine::Lazy(_) => todo!(),
//         }
//         todo!()
//     }
...
//     // mm and fa
//     pub fn mm(&self, other: &Tensor) -> Result<Tensor, OpForwardError> { unimplemented!() }
