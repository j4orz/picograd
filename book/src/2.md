...
The only difference as we'll shortly see soon is that the inductive bias used to model the function $f$ includes *non-linearities* with many layers of *composition*.
...
Recall that the task of house price prediction and sentiment classification which took the form of
some function $f:\reals^d \to \reals$ and $f:\reals^d \to [0,1]$ respectively.
...
because the log odds produced by the inner product (which is indeed *affine*)
...
The answer is technically *yes*, it can be considered a *degenerative* deep neural network with $0$ *hidden layers* <span class="sidenote-number"></span><span class="sidenote">Very much in the same way that a *list* can be considered a degenerative binary *tree* or *graph*</span>.
Besides including non-linearities, these so-called hidden layers are the second aspect to deep neural networks which differentiates them from general linear models,
where each hidden layer successively *lifts the representation of data* so that for the same task of recovering a function of the form $f:\reals^d \to \reals$,
the function is modeled as $f(\mathbf{x}):= h^{[l]} \circ h^{[l-1]} \circ \cdots h^{[1]} \circ h^{[0]}(\mathbf{x})$,
...
Let's now turn out attention to the function bodies of these functions $h$ with a deep neural netork of $1$ hidden layer,
carrying out the task of price regression so that $f$ has the form $f:\reals^d\to\reals$.
