...
- [2.2 Programming Iterative Optimizers and Automatic Differentation with `optim.sgd` and `Tensor.backward()`](#16-programming-teenygrad-level-1-optimization-and-differentiation)
...
- [2.3 Learning Sequences with Different Inductive Biases in `torch`](#24-learning-sequences-with-different-inductive-biases)
  - [2.3.1 Learning Sequences with Convolutional Neural Networks (CNNs)]()
  - [2.3.2 Learning Sequences with Recurrent Neural Networks (RNNs)]()
  - [2.3.3 Learning Sequences with Bidirectional Encoder Representations from Transformers (BERTs)]()
  - [2.3.4 Learning Sequences with Generative Pretrained Transformers (GPTs)]()
- [2.4 Accelerating `cuBLAS` Kernels with `teenygrad`](#23-accelerating-cublas-kernels)
  - [2.4.1 Programming Massively Parallel Processors]()
  - [2.4.2 Accelerating `GEMM` from `CUDA Rust` to `PTX`]()
  - [2.4.3 Accelerating `GEMM` on GPU with Data Reuse]()
  - [2.4.4 Accelerating `GEMM` on GPU with Scheduling]()
  - [2.4.5 Accelerating `GEMM` on GPU with Tensor Cores]()
...
## 2.2 Programming Optimizers and Automatic Differentation with `optim.sgd` and `Tensor.backward()`
