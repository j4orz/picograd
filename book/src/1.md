...
the discipline of programming practiced in Silicon Valley shares the same dream of mathematics in Ancient Greece:
...
they must make a transition to programming from *discrete objects* using *deterministic functions* to that of *continous objects* $x$ using *stochastic functions* $f(x)$.
...
Thus far, you've composed programs with data structures and algorithms<span class="sidenote-number"></span><span class="sidenote">*Perhaps with classic canon like [Algorithms + Data Structures = Programs](https://archive.org/details/algorithmsdatast0000wirt)*</span>,
which in turn are composed of data and function<span class="sidenote-number"></span><span class="sidenote">*Maybe with classic canon like [Structure and Interpretation of Computer Programs](https://mitp-content-server.mit.edu/books/content/sectbyfn/books_pres_0/6515/sicp.zip/index.html)*</span>.
...
by representing types continuously with coordinates and distributing truth stochastically with probability.
Then, in [1.2 Statistical Learning of *Continuously Stochastic Maps* with `numpy`](),
and [1.3 Statistical Learning of Continuously Stochastic *Classifying* Maps with `numpy`]()
we will learn how to recover *stochastic functions* $y=f(\mathbf{x})$ of software 2.0 by recovering stochastic maps with linear transformations.

From there, we will change gears as *users* of numerical libraries to that of *implementors*
by developing the first pieces of our own deep learning framework `teenygrad`, which serves as the book's capstone project<span class="sidenote-number"></span><span class="sidenote">*In part two we will evolve teenygrad to support neural networks and gpu acceleration. In part three, to support that of [nanogpt]() and [nanochat]()*</span>.
This involves programming the user-visible mathematics of linear algebra, optimization, and differentiation
with `teenygrad.Tensor`, `teenygrad.optim.sgd` and `teenygrad.Tensor.backward()` in the "productive programming language" of Python,
but it also requires diving into the *soul of the machine* with the "systems programming language"<span class="sidenote-number"></span><span class="sidenote">*"systems programming language" with quotes because taxonifying languages into paradigms such as "functional", "imperative", "object-oriented", "web", "systems" is non-sensical. To learn more, read Krishnamurthi's [Teaching Programming Languages in a Post-Linnaean Age](https://cs.brown.edu/~sk/Publications/Papers/Published/sk-teach-pl-post-linnaean/) from SIGPLAN 2008.*</span> of Rust.
in order to accelerate said basic linear algebra subroutines defined on `teenygrad.Tensor`.
After completing
...
followed by [1.5 Programming `teenygrad` *Level 0*: Accelerating the `BLAS` on CPU](#15-programming-teenygrad-level-0-accelerating-the-blas-on-cpu),
and finally [1.6 Programming `teenygrad` *Level 1*: Optimization and Differentiation](#16-programming-teenygrad-level-1-optimization-and-differentiation),
your `teenygrad` will be able to run the same linear models developed in the first three chapters,
and will be prepared for [part two](./2.md) of the book which adds support for deep neural networks and GPU acceleration.

## Table of Contents
- [1.1 Statistical Inference of Continuously Stochastic *Data* with `Python`](#11-statistical-inference-of-stochastic-data-with-python)
...
- [1.3 Statistical Learning of Continuously Stochastic *Classifying Maps* with `numpy`](#13-statistical-learning-of-continuously-stochastic-classifying-maps-with-numpy)
...
- [1.4 Programming `teenygrad` *Level -1*: Tensor, the Multidimensional Array](#14-programming-teenygrad-level--1-high-dimensional-data-and-function)
  - [1.4.1 The Simplest `Tensor` Implementation with Nested `list`s]()
  - [1.4.2 Improving the Locality of `Tensor` by Decoupling Copies and Views with Strides]()
  - [1.4.3 Implementing Basic Linear Algebra Subroutines: `SAXPY`, `SGEMV` and `SGEMM`]()
  - [1.4.4 Accelerating `CPython` with Extension Modules via `PyO3` and `Rust`]()
  - [1.4.5 Accelerating `GEMM` on CPU with `Rust`]()
- [1.5 Programming `teenygrad` *Level 0*: Accelerating the `BLAS` on `RISC-V` CPUs with `rustc`](#15-programming-teenygrad-level-0-accelerating-the-blas-on-risc-v-cpus-with-rustc)
  - [1.5.1 A Programmer's Perspective on Computation and Communication: Pipelines and Hierarchies]()
  - [1.5.2 Bottleneck Analysis of Throughput Performance with Arithmetic Intensity of Rooflines]()
  - [1.5.3 Accelerating Communication of the `BLAS` with Memory Hierarchies]()
  - [1.5.4 Accelerating Computation of the `BLAS` with Instruction Level Parallelism]()
  - [1.5.5 Accelerating Computation of the `BLAS` with Data Level Parallelism]()
  - [1.5.6 Accelerating Computation of the `BLAS` with Thread Level Parallelism]()
- [1.6 Programming `teenygrad` *Level 1*: Optimization and Differentiation](#16-programming-teenygrad-level-1-optimization-and-differentiation)
  - [1.6.1 Symbolic, Numerical, and Automatic Differentiation]()
  - [1.6.2 Forward Mode Automatic Differentiation with `Tensor.forward()`]()
  - [1.6.3 Reverse Mode Automatic Differentiation with `Tensor.backward()`]()
  - [1.6.4 Iterative Optimization with Stochastic Gradient Descent `optim.sgd`]()
...
### 1.3.1 Linear Regression, and the Limitations of Direct Methods


### 1.3.2 Inductive Bias and Principle with Logistic Regression and Cross Entropy

### 1.3.3 Iterative Optimization via Gradient Descent



### 1.3.4 Multinomial Logistic Regression
### 1.3.5 Generalized Linear Models
...
## 1.5 Programming `teenygrad` *Level 0*: Accelerating the `BLAS` on `RISC-V` CPUs with `rustc`
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
<span class="sidenote-number"></span><span class="sidenote">*To sidetrack to familiarize yourself with systems programming in the context of software 1.0 , read the classic of [Computer Systems A Programmer's Perspective](https://csapp.cs.cmu.edu/) or it's [spiritual successor](https://www.cs.cornell.edu/courses/cs3410/2024fa/) which provides updated lecture notes for the RISC-V instruction set architecture*</span>with instruction set architectures, microarchitecture, memory hierarchies
<span class="sidenote-number"></span><span class="sidenote">*This book presents the computation of microarchitecture and communication of memory hierchies from a system programmer's perspective (the one's programming frameworks like `numpy`, `torch`, and `jax`). If you are interested in hardware engineering, take a look at 
[Computer Organization and Design]() and [Computer Architecture: A Quantitative Approach]() which both employ the usage of RISC-V.*</span>.

- instruction set architecture
- control path, data path, memory, input, output.
...
### 1.5.3 Accelerating `GEMM` on CPU with Locality of Memory Hierarchies
### 1.5.4 Accelerating `GEMM` on CPU with Instruction Level Parallelism
### 1.5.5 Accelerating `GEMM` on CPU with Data Level Parallelism
### 1.5.6 Accelerating `GEMM` on CPU with Thread Level Parallelism
