...
Thus far, you've composed programs with data structures and algorithms<span class="sidenote-number"></span><span class="sidenote">*Perhaps with classic canon like Algorithms + Data Structures = Programs*</span>,
which in turn are composed of data and function<span class="sidenote-number"></span><span class="sidenote">*Maybe with classic canon like Structure and Interpretation of Computer Programs*</span>.
...
but it also requires diving into the *soul of the machine* with a "systems programming language"<span class="sidenote-number"></span><span class="sidenote">*"systems programming language" with quotes because taxonifying languages into paradigms such as "functional", "imperative", "object-oriented", "web", "systems" is non-sensical. To learn more, read Krishnamurthi's [Teaching Programming Languages in a Post-Linnaean Age](https://cs.brown.edu/~sk/Publications/Papers/Published/sk-teach-pl-post-linnaean/) from SIGPLAN 2008.*</span>
in order to accelerate said basic linear algebra subroutines defined on `teenygrad.Tensor`.
This book uses Rust<span class="sidenote-number"></span><span class="sidenote">*If you want to backtrack or sidetrack and familiarize yourself with Rust, take a look at the enhanced version of [The Rust Programming Language](https://rust-book.cs.brown.edu/) by Will Crichton and Shriram Krishnamurthi.*</span>, but feel free to follow along with C/C++<span class="sidenote-number"></span><span class="sidenote">*In that case, take a look at "The C Programming Language by Brian Kernighan and Dennis Ritchie.*</span> — what's fundamental to the purpose of accelerating said basic linear algebra subroutines on the CPU is
...
and will be prepared for [part two]() of the book which adds support for deep neural networks and GPU acceleration.
...
- [1.4 Programming `teenygrad` *Level -1*: Tensor, the Multidimensional Array](#14-programming-teenygrad-level--1-high-dimensional-data-and-function)
  ...
  - [1.4.5 Accelerating `CPython` with Extension Modules via `PyO3` and `Rust`]()
  - [1.4.6 Accelerating `GEMM` on CPU from `Rust` to `x86` with `rustc`]()
- [1.5 Programming `teenygrad` *Level 0*: Accelerating the `BLAS` on CPU](#15-programming-teenygrad-level-0-accelerating-the-blas-on-cpu)
  - [1.5.1 A Programmer's Perspective on Computation and Communication]()
  - [1.5.2 Throughput Performance Bottleneck Analysis with Arithmetic Intensity of Rooflines]()
  - [1.5.3 Accelerating `GEMM` CPU Communication with Memory Hierarchies]()
  - [1.5.4 Accelerating `GEMM` CPU Computation with Instruction Level Parallelism]()
  - [1.5.5 Accelerating `GEMM` CPU Computation with Data Level Parallelism]()
  - [1.5.6 Accelerating `GEMM` CPU Computation with Thread Level Parallelism]()
...
### 1.4.5 Accelerating `GEMM` on CPU from `Rust` to `x86` with `rustc`
```rust
...
## 1.5 Programming `teenygrad` *Level 0*: Accelerating the `BLAS` on CPU
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>

### 1.5.1 A Programmer's Perspective on Computation and Communication
Here is some text that needs a note<span class="sidenote-number"></span><span class="sidenote">*This appears in the margin, just like Tufte.*</span> and then the sentence continues.
