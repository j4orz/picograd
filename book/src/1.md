...
- [1.5 Programming `teenygrad` *Level 0*: Accelerating the `BLAS` with `rustc` and `x86`](#15-programming-teenygrad-level-0-accelerated-basic-linear-algebra-subroutines)
  - [1.5.1 Evaluating Chips and Translating Compilers: A Programmer's Perspective]()
  - [1.5.2 A Quantitative Approach: Benchmarks, Rooflines, and Profiles]()
  - [1.5.3 Accelerating `CPython` with Extension Modules via `PyO3` and `Rust`]()
  - [1.5.4 Accelerating `GEMM` on CPU with Locality of Memory Hierarchies]()
  - [1.5.5 Accelerating `GEMM` on CPU with Instruction Level Parallelism]()
  - [1.5.6 Accelerating `GEMM` on CPU with Data Level Parallelism]()
