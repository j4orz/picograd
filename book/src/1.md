...
- [1.5 Programming `teenygrad` *Level 0*: Accelerating the `BLAS`](#15-programming-teenygrad-level-0-accelerated-basic-linear-algebra-subroutines)
  - [1.5.1 A Programmer's Perspective on Computation and Communication]()
  - [1.5.2 A Quantitative Approach to Performance with Benchmarks and Rooflines]()
...
- [1.6 Programming `teenygrad` *Level 1*: Optimization and Differentiation](#16-programming-automatic-differentiation-and-gradient-descent)
  ...
  - [1.6.2 Forward Mode Automatic Differentiation with `Tensor.backward()`]()
...
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
### 1.5.1 A Programmer's Perspective on Computation and Communication
Here is some text that needs a note<span class="sidenote-number"></span><span class="sidenote">*This appears in the margin, just like in that CUDA blog post.*</span> and then the sentence continues.
