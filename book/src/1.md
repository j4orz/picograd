...
by building `teenygrad` from scratch — after three chapters we will have built our very own CPU accelerated multidimensional array with automatic differentiation
...
<span class="sidenote-number"></span><span class="sidenote">*The opensource RISC-V `BOOM` core and it's RVV extension fork `ocelot` will be used to introduce instruction sets and microarchitecture,
but x86 machines like Intel's Xeon and AMD's EPYC will be used to achieve server-class performance.*</span>
...
- [1.4 Programming `teenygrad` *Level -1*: Tensor, the Multidimensional Array](#14-programming-teenygrad-level--1-high-dimensional-data-and-function)
  ...
  - [1.4.4 Accelerating `GEMM` on CPU with `Rust`]()
