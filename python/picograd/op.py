...
class OpCode(FastEnum):
  ...
  CONTIGUOUS = auto(); CONTIGUOUS_BACKWARD = auto(); DETACH = auto() # opCode that adjust the behavior of the scheduler

  # movement opCode! these only exist in the tensor graph
  RESHAPE = auto(); PERMUTE = auto(); EXPAND = auto(); PAD = auto(); SHRINK = auto(); FLIP = auto()
  MULTI = auto()  # MULTI is really a movement op

  DEFINE_GLOBAL = auto(); DEFINE_LOCAL = auto(); DEFINE_REG = auto() # TODO: unify these opCode into the levels of the memory hierarchy. depends on ASSIGN is STORE
  ...
  # UNROLL = auto(); CONTRACT = auto(); GEP = auto(); VECTORIZE = auto(); CAT = auto(); PTRCAT = auto() # optimization helper opCode
  CAST = auto(); BITCAST = auto(); EXP2 = auto(); LOG2 = auto(); SIN = auto(); SQRT = auto(); RECIPROCAL = auto(); NEG = auto(); TRUNC = auto() # UnaryOpCode
  ...
  WHERE = auto(); MULACC = auto() # ternaryopCode
  BARRIER = auto(); RANGE = auto(); IF = auto(); END = auto(); ENDIF = auto() # control flow opCode
...
class Op(ComputeMixin): # MovementMixin, metaclass=UOpMetaClass
  ...
  def toposort(self, gate:Callable|None=None) -> dict[Op, None]:
    ...
    while stack:
      ...
      if node in output: continue
      if not visited:
        if gate is None or gate(node):
          stack.append((node, True))  # push node back on stack to process after its srcs
          for s in reversed(node.src): stack.append((s, False)) # push srcs on the stack
      else: output[node] = None # second time i'm seeing this node, add it to returned toposort
    return output